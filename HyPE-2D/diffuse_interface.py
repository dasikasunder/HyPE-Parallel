...
def plot_tecplot(in_file, out_file):
    ...
    print("TITLE = \"Diffuse Interface Euler Equations. File created on: ", dt_string, "\"",  file = outfile)
    outfile.write("VARIABLES = \"x\", \"y\", \"Density\", \"x-Velocity\", \"y-Velocity\", \"Pressure\",\"grad_RHO\" ")
    print('\nZone I = ', N_x, "J = ", N_y, file = outfile) 
    
    for j in range(0, N_y):
        for i in range (0, N_x):
            print(coords[j,i,0], coords[j,i,1], W[j,i,4]*W[j,i,0], W[j,i,4]*W[j,i,1], W[j,i,4]*W[j,i,2], W[j,i,4]*W[j,i,3], grad_RHO[j,i], file = outfile)
